...
export function CourseCard({ course }: { course: Course }) {
  const notesHref = `/student/notes?course=${course.id}`;
  const quizzesHref = `/student/quizzes?course=${course.id}`;
  ...
  return <article className="flex min-w-0 flex-col gap-6 rounded-xl border border-[#DCE6F2] bg-white p-5 shadow-[0_8px_24px_rgba(46,93,145,0.04)] sm:p-6">
    ...
    <div className="flex w-full min-w-0 flex-col gap-3 lg:flex-row"><Link href={notesHref} className="inline-flex min-h-[60px] min-w-0 flex-1 items-center justify-center gap-3 rounded-xl bg-[#E8F1FF] px-5 text-sm font-bold text-[#1269D8] transition hover:bg-[#DCEAFF] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"><FileIcon className="h-6 w-6 shrink-0" />Notes &amp; PDFs</Link><Link href={quizzesHref} className="inline-flex min-h-[60px] min-w-0 flex-1 items-center justify-center gap-3 rounded-xl bg-[#287BF0] px-5 text-sm font-bold text-white shadow-[0_7px_14px_rgba(37,99,235,0.18)] transition hover:bg-[#1769D9] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"><PencilIcon className="h-6 w-6 shrink-0" />Quizzes</Link></div>
  </article>;
}
